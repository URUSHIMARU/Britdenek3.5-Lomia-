...
execute if entity @e[tag=boss13,tag=body,scores={motion=22..30}] as @e[tag=right_arm,limit=1,tag=boss13] at @s run teleport @s ~ ~ ~ ~-8 ~-4
...
execute if entity @e[tag=boss13,tag=body,scores={motion=40..80}] as @e[tag=right_arm,limit=1,tag=boss13] at @s run particle minecraft:block{block_state:"minecraft:raw_gold_block"} ^ ^1 ^1.6 0.5 0.5 0.5 1 5 force
execute if entity @e[tag=boss13,tag=body,scores={motion=22..80}] as @e[tag=right_arm,limit=1,tag=boss13] at @s run particle minecraft:witch ^ ^1 ^1.6 0.2 0.2 0.2 1 1 force

execute if entity @e[tag=boss13,tag=body,scores={motion=40}] as @e[tag=body,limit=1,tag=boss13] at @s run summon minecraft:armor_stand ~ ~ ~ {Tags:["boss13_attack","boss13_rance"],Marker:1b,Invisible:1b}


execute if entity @e[tag=boss13,tag=body,scores={motion=40}] as @e[limit=1,tag=boss13_rance] at @s run loot spawn ~ ~ ~ loot namespace:random/random3
execute if entity @e[tag=boss13,tag=body,scores={motion=40}] as @e[limit=1,tag=boss13_rance] at @s if entity @e[nbt={Item:{count:1,components:{"minecraft:custom_data":{random:1}}}},type=item,distance=..0.1] run tag @s add boss13_rance1
execute if entity @e[tag=boss13,tag=body,scores={motion=40}] as @e[limit=1,tag=boss13_rance] at @s if entity @e[nbt={Item:{count:2,components:{"minecraft:custom_data":{random:1}}}},type=item,distance=..0.1] run tag @s add boss13_rance2
execute if entity @e[tag=boss13,tag=body,scores={motion=40}] as @e[limit=1,tag=boss13_rance] at @s if entity @e[nbt={Item:{count:3,components:{"minecraft:custom_data":{random:1}}}},type=item,distance=..0.1] run tag @s add boss13_rance3


execute as @s at @s positioned ~ ~ ~ run kill @e[type=item,distance=..3]
execute if entity @e[tag=boss13,tag=body,scores={motion=70}] as @e[tag=right_arm,limit=1,tag=boss13] at @s run teleport @s ~ ~ ~ ~80 ~-30
