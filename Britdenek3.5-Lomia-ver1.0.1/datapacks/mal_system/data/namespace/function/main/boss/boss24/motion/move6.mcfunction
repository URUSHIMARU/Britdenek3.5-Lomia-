...
execute as @e[tag=boss24,tag=body,limit=1,scores={motion=10..16}] at @s if block ^ ^0.5 ^2 #namespace:airs_for_mob run teleport @s ^ ^ ^0.8
...
execute if entity @e[tag=boss24,tag=body,scores={motion=17}] as @e[tag=boss24,tag=body] at @s positioned ^ ^ ^2 as @e[tag=boss24battle,distance=..4] unless score @s player = @e[tag=boss24,tag=body,limit=1] player run scoreboard players set @s commandDamage 13
...
execute as @e[tag=boss24,tag=body,scores={motion=40..},limit=1] at @s run function namespace:main/boss/boss24/motion/random1
