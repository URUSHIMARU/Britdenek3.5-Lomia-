...
execute as @e[tag=boss25,tag=body,limit=1,scores={motion=10..16}] at @s if block ^-2 ^0.5 ^ #namespace:airs_for_mob run teleport @s ^-0.8 ^ ^
...
execute as @e[tag=boss25,tag=body,scores={motion=20..},limit=1] at @s run function namespace:main/boss/boss25/motion/random2

execute if entity @e[tag=boss25,tag=body,scores={motion=20..},limit=1] run scoreboard players set @e[tag=boss25,tag=body] motion 0
