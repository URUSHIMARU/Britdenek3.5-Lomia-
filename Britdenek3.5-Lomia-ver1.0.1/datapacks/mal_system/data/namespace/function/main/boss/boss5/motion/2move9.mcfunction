...
#ÉÇÅ[ÉVÉáÉì
execute if entity @e[tag=boss5,tag=body,scores={motion=1..25}] as @e[tag=boss5] at @s unless block ~ ~-1.5 ~ #namespace:airs run tag @e[tag=boss5,tag=body,limit=1,tag=!lock] add lock
execute if entity @e[tag=boss5,tag=body,scores={motion=1..25}] as @e[tag=boss5,tag=body,limit=1,tag=!lock] at @s run teleport @s ~ ~-0.25 ~ 



scoreboard players set @e[tag=boss5,tag=body,scores={motion2=25..},limit=1] motion2 0

execute if entity @e[tag=boss5,tag=body,scores={motion=25..},limit=1] run tag @e[tag=body,limit=1,tag=boss5] remove lock
